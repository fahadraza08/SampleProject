trigger RejectDoubleBooking on Session_Speaker__c (before insert, before update) {
    ...
    try{
        ...
        for(Session__c related_session : related_sessions) {
            requested_bookings.put(related_session.Id,related_session.Session_Date__c);        }    }catch(Exception e){}
    List<Session_Speaker__c> related_speakers = new List<Session_Speaker__c>();
    try{
        //get related speaker sessions to check against
        related_speakers = [SELECT ID, Speaker__c, Session__c, Session__r.Session_Date__c from Session_Speaker__c WHERE Speaker__c IN :speakerIds];    }catch(Exception e){}
